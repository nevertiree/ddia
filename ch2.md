...
数据模型可能是开发软件最重要的部分，因为它们有着深远的影响：既影响软件的编写方式，又影响我们的解题思路。

大多数应用程序是通过将一个数据模型叠加在另一个之上来构建的。每一层的关键问题是：如何用下一层来表示自己？举例来说：

1. 作为一名应用程序开发人员，您将看到现实世界（包括人员、组织、货物、行为、资金流向、传感器等），并根据对象、数据结构、能够操纵数据结构的API进行建模。这些结构通常是应用程序特定的。
2. 如果想要存储这些数据结构，我们可以使用通用数据模型（例如JSON/XML文档、关系数据库中的表、图模型）来表示它们。
3. 构建数据库软件的工程师决定以内存、磁盘或网络上的字节来表示JSON/XML/关系/图数据。该表示可以允许以各种方式来查询、搜索、操纵和处理数据。
...
目前有许多不同类型的数据模型，每个数据模型都实现了自己的设计目的。某些用法很容易，有些不被支持;有些操作很快，有些操作不好;有些数据转换感觉自然，有些令人感到尴尬。

一个人可能需要很多努力才能精通一个数据模型（想想看仅关系数据建模就有多少本书）。即使只使用一种数据模型，而不用担心其内部工作，构建软件也是非常困难的。但是由于数据模型对软件的功能有很大的影响，因此选择适合应用程序的数据模型是非常重要的。

在本章中，我们将研究一系列用于数据存储和查询的通用数据模型（前面列表中的第2点）。特别地是，我们将比较关系模型、文档模型和图数据模型。我们还将浏览各种查询语言并比较它们的用例。在第3章中，我们将讨论存储引擎是如何工作的，或者说，这些数据模型是如何实际实现的（列表中的第3点）。



## 关系模型与文档模型

现在最着名的数据模型可能是SQL，它基于Edgar Codd在1970年提出的关系模型【1】：即数据被组织到关系中（关系又被称为SQL表），而这种关系是元组的无序集合（元组被称为SQL行）。

关系模型曾经只是个理论提议，当时很多人都怀疑是否能够有效地实现它。然而到了20世纪80年代中期，关系数据库管理系统（RDBMS）和SQL已成为大多数需要存储和查询具有某种规模结构的数据的人们的首选工具。关系数据库的优势已经持续了长达25到30年，这在计算机历史上已经可以被称为永恒了。

关系数据库起源于商业数据处理，这是在20世纪60年代和70年代在大型计算机上进行的。从今天的角度来看，这种用例显得很平常：通常是**事务处理**（进入销售或银行交易、航空公司预订、仓库库存）和**批处理**（客户发票、工资单、报告）。

当时的其他数据库迫使应用程序开发人员考虑数据库内部的数据表示，而关系模型的目标是将实现细节隐藏在更简洁的界面之后。

多年来，在数据存储和查询方面存在着许多相互竞争的方法。在20世纪70年代和80年代初，网络模型和分层模型是主要的选择，但关系模型占据了主导地位。对象数据库在20世纪80年代末和90年代初再次出现。 XML数据库出现在21世纪初，但只有小众采用。关系模型的每个竞争者都在其时代大量地炒作自己，但泡沫没有持续多久【2】。

随着电脑越来越强大和互联，它们开始被用于日益多样化的目的。值得注意的是，关系数据库的使用场景被很广泛地推广，远远超出了业务数据处理这一原始范围。您今天在网上看到的大部分内容仍然是由关系数据库提供支持，无论是在线发布、讨论、社交网络、电子商务、游戏、软件即服务生产力应用程序等等。

### NoSQL的诞生

在2010年代的今天，NoSQL是推翻关系模式主导地位的一次最新尝试。 “NoSQL”这个名字非常不幸，因为它实际上并没有涉及到任何特定的技术，NoSQL最初只是在2009年的一个关于分布式、非关系数据库的开源聚会上被提出的吸引眼球的Twitter标签。无论如何，这个术语触动了某些神经，并迅速传遍了网络社区。现今，一些有趣的数据库系统现在与*#NoSQL#*标签相关联，并被追溯性地重新解释为“不仅是SQL” 【4】。
...
* 对关系模型限制性感到受挫，渴望更多动态性与表现力

不同的应用程序有不同的要求，一个用例的最佳选择可能不同于另一个用例的最佳选择。因此，在可预见的未来，关系数据库似乎可能会继续与各种非关系数据库一起使用 —— 这种想法有时也被称为**混合持久化（Polyglot Persistences）**

### 对象关系不匹配

现在大多数应用程序开发都是在面向对象的编程语言中完成的，这导致了对SQL数据模型的普遍批评：如果数据存储在关系表中，那么应用程序代码中的对象和数据库模型的表、行、列之间需要一个笨拙的转换层。模型之间的不连贯有时被称为**阻抗不匹配（impedance mismatch）**[^i]。

[^i]: 从电子学借用一个术语。每个电路的输入和输出都有一定的阻抗（交流电阻）。当您将一个电路的输出连接到另一个电路的输入时，如果两个电路的输出和输入阻抗匹配，则连接上的功率传输将被最大化。阻抗不匹配可能导致信号反射和其他问题

像ActiveRecord和Hibernate这样的**对象关系映射（object-relational mapping, ORM）**框架减少了这个转换层需要的样板代码的数量，但是它们不能完全隐藏这两个模型之间的差异。

![](img/fig2-1.png)

**图2-1 使用关系型模式来表示LinkedIn简历**

例如，[图2-1](img/fig2-1.png)展示了如何在关系模式中表达简历（一个LinkedIn简介）。整个简历可以通过唯一的标识符`user_id`来标识，像`first_name`和`last_name`这样的字段每个用户只出现一次，所以他们可以在用户表上建模为列。但是，大多数人有多于一次的职业经历，同时人们可能有不同的教育经历和不同数量的联系方式。从用户到这些字段之间存在一对多的关系，目前可以用多种方式来表示：

* 在传统SQL模型（SQL1999之前）中，最常见的规范化表示形式是将职位、教育经历和联系方式放在单独的表中，对用户表提供外键引用，如[图2-1](img/fig2-1.png)所示。
...
* 第三种选择是将职位，教育和联系方式编码为JSON或XML文档，将其存储在数据库的文本列中，并让应用程序解释其结构和内容。在这种配置中，通常不能使用数据库查询该编码列中的值。
...
一些开发人员认为JSON模型减少了应用程序代码和存储层之间的阻抗不匹配。缺乏模式往往被认为是一个优势，我们将在“[文档模型中的模式灵活性](#文档模型中的模式灵活性)”中详细讨论。但是，正如我们将在[第4章](ch4.md)中看到的那样，JSON作为数据编码格式也存在问题。

JSON表示比[图2-1](img/fig2-1.png)

中的多表模式具有更好的局部性（locality）。如果要在关系数据中获取简历，则需要执行多个查询（通过`user_id`查询每个表），或者在用户表与其下属表之间执行混乱的多路连接。在JSON表示中，所有的相关信息都在一个地方，一个查询就足够了。

从用户简历到用户位置、教育历史和联系方式的一对多关系意味着数据中的树状结构，而JSON表示使得这个树状结构变得明确（见[图2-2](img/fig2-2.png)）。
...
在上一节的[例2-1]()中，为什么`region_id`和`industry_id`是以ID而不是纯字符串“大西雅图地区”和“慈善”的形式给出的？

如果用户界面具有用于输入区域和行业的自由文本字段，则将其存储为纯文本字符串是有意义的。但是，对地理区域和行业进行标准化，并让用户从下拉列表或自动填充器中进行选择是有好处的：

* 统一的样式和拼写
* 避免歧义（例如，如果有几个同名的城市）
* 易于更新——名称只存储在一个地方，所以如果需要更改（例如城市名称的改变），便于全面更新。
* 本地化支持——当网站翻译成其他语言时，标准化的名单可以被本地化，所以地区和行业可以使用用户的语言来表示
* 更好的搜索——例如，搜索华盛顿州的慈善家可以匹配这份简历，因为地区列表可以编码记录西雅图在华盛顿的事实（从“大西雅图地区”这个字符串中看不出来）

无论是存储一个ID还是一个文本字符串，都是一个关于**重复**的问题。当你使用一个ID时，对人类有意义的信息（比如单词：慈善）只存储在一个地方，引用它的所有信息都使用一个ID（ID只在数据库中有意义）。当你直接存储文本时，该信息将会在每个使用它的记录中被重复。

使用ID很有好处，因为ID对人类没有任何意义，所以它永远不需要改变：即ID可以保持不变，即使它标识的信息发生变化。因为任何对人类有意义的信息都可能需要在将来某个时候改变，因此如果这些信息被广泛复制，那么所有的冗余副本都需要更新。这种更新一方面会导致写入开销，一方面会存在不一致风险（信息的一些副本被更新，但其他信息的副本不被更新）。去除这种重复是数据库规范化的关键思想。（关系模型区分了几种不同的范式，但这些区别实际上并不重要。 

[^ii]: 关于关系模型的文献区分了几种不同的规范形式，但这些区别几乎没有实际意义。 作为一个经验法则，如果您重复了只能存储在一个地方的值，那么当前的模式就不是**规范化（normalized）**的[^ii]。）
...
如果数据库本身不支持连接，则必须通过对数据库进行多个查询来模拟应用程序代码中的连接。（在这种情况下，地区和行业的名单应该很小并且变化不大，可以让应用程序简单地将它们读入内存。但是不管怎么说，建立联接的工作还是从数据库转移到了应用程序代码。）

而且，即使应用程序的初始版本适合无连接的文档模型，但随着更多功能添加到应用程序中，数据也会变得更加互联。下面我们可以对简历例子进行一些修改：

***组织和学校作为实体***

在前面的描述中，`organization`（用户工作的公司）和`school_name`（他们学习的地方）只是字符串。也许他们应该是对实体的引用呢？然后，每个组织，学校或大学都可以拥有自己的网页（包括标识、新闻提要等）。每个简历可以链接到它所提到的组织和学校，并且包括他们的标识和其他信息（参见[图2-3](img/fig2-3.png)，来自LinkedIn的一个例子）。
